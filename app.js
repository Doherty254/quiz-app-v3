...
app.post("/quiz", (req, res) => {
  ...
  let sql =
    "INSERT INTO score (s_id_fk, response, results) VALUES (?,JSON_ARRAY(?),?)";
  connection.query(
    sql,
    [
      req.session.userID,
...
app.post("/edit-profile/:id", uploads.single("picture"), (req, res) => {
  if (req.file) {
    let sql =
      "UPDATE student SET email = ?, name = ?, gender = ?, dob = ?, picture = ?, contacts = ? WHERE s_id = ?";
    ...
    let sql =
      "UPDATE student SET email = ?, name = ?, gender = ?, dob = ?, contacts = ? WHERE s_id = ?";
